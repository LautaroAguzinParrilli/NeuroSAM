...
# -------- data --------
    ...
    val_df, test_df = train_test_split(val_df, test_size=0.5, random_state=42) #training on full database and testing on externals
...
# -------- training --------
    criterion = nn.MSELoss() #funcion de costo
    optimizer = torch.optim.AdamW( #optimizador
        model.parameters(),  
        lr=1e-4,
    ...
    for epoch in range(num_epochs):
        ...
        model.train()
        train_loss = 0.0
        for imgs, ages in train_loader: #itera sobre los batches del dataloader
            imgs = imgs.to(device)
            ages = ages.to(device).unsqueeze(1)
            preds = model(imgs)
            loss = criterion(preds, ages)

            optimizer.zero_grad() #reseter los gradientes
            loss.backward() #calcula los gradientes
            optimizer.step() #actualiza los pesos del modelo en funcion del gradiente
            train_loss += loss.item() * imgs.size(0)

        train_loss /= len(train_loader.dataset) #promedio del loss por muestra
        ...
        scheduler.step(val_loss) #ajusta el learning rate en funcion de la val_loss
        ...
        if val_loss < best_val_loss:
            ...
            epochs_no_improve = 0
            best_model_state = model.state_dict() #guardo el modelo con la mejor val_loss
        else:
            epochs_no_improve += 1
            if epochs_no_improve >= patience: #si no hay mejoras durante 'patience' épocas, detengo el entrenamiento 
    ...
    torch.save(model.state_dict(), 'model.pth') #guardo el modelo entrenado
